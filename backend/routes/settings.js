...
// IMPORTANT: use the shared pool (NOT mysql.createConnection)
...
router.get("/voting-date", async (req, res) => {
  try {
    ...
    console.error("Voting date fetch error:", err);
    res.status(500).json({ error: "Failed to fetch voting date" });
  }
});
...
router.get("/voting-window", async (req, res) => {
  try {
    ...
    console.error("Voting window fetch error:", err);
    res.status(500).json({ error: "Failed to fetch voting window" });
  }
});
